...
public class ZippyLights2D : MonoBehaviour
{
	...
	void OnEnable() {
		...
		if (!Application.isPlaying) {
			...
			Init();
			Brighten();
			EditorUtility.SetSelectedWireframeHidden(cacheMeshRenderer, true);
		} else {
			EditorApplication.update -= LateUpdate;
		}
#endif
	...
	void FindOrCreateLightObject() {
		...
		if (duplicatedLights != null && lightMesh != null) {
			for (int i = 0; i < duplicatedLights.Count; i++) {
				duplicatedLights[i].sharedMesh = lightMesh;
				MeshRenderer dm = duplicatedLights[i].GetComponent<MeshRenderer>();
#if UNITY_EDITOR
				EditorUtility.SetSelectedWireframeHidden(dm, true);
#endif
